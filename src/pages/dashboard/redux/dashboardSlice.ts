  ...
  reducers: {
    ...
    playerListSuccess: (state, action) => {
      ...
      if (action.payload?.pagination) {
        ...
        state.nextPage = action.payload.pagination.hasNextPage
          ? (state.nextPage ?? 0) + 1
          : (state.nextPage ?? 0);
        state.previousPage = action.payload.pagination.hasPrevPage
          ? (state.previousPage ?? 0) + 1
    ...
    // Delete agent reducers
    deleteAgentRequest: (state, action: PayloadAction<string>) => {
      state.loading = true;
    },
    deleteAgentSuccess: (state) => {
      state.loading = false;
    },
    deleteAgentFailure: (state) => {
      state.loading = false;
    },
    // update agent reducers
    updateAgentRequest: (state, action: PayloadAction<AgentCreationData>) => {
      state.loading = true;
    },
    updateAgentSuccess: (state) => {
      state.loading = false;
    },
